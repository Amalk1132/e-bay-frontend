...
const Oral = () => {
  ...
  return (
    <div className="main-oral">
      {data
        .filter((item) => item.Subcategory === "oralcare")
        .map((item, index) => {
          return (
            
              
              <div key={index} className="sub-oral">
                <div className="oral-img">
                <Link to={`/displayprod/${item._id}`}> <img src={item.Image} alt="oralcare" /></Link>
                </div>

                <div className="oral-part">
                  <h1>{item.Title}</h1>
