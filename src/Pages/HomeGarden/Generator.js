...
const Generator = () => {
  ...
  return (
    <div className="main-gen">
      {data
        .filter((item) => item.Subcategory === "generator")
        .map((item, index) => {
          return (
           
             
              <div key={index} className="sub-gen">
                <div className="gen-img">
                <Link to={`/displayprod/${item._id}`}> <img src={item.Image} alt="generator" /> </Link>
                </div>

                <div className="gen-part">
                  <h1>{item.Title}</h1>
