...
const Womeglass = () => {
  ...
  return (
    <div className="main-womenglass">
      {data
        .filter((item) => item.Subcategory === "womenglass")
        .map((item, index) => {
          return (
          
              <div key={index} className="sub-womenglass">
                <div className="wogls-img">
                <Link to={`/displayprod/${item._id}`}> <img src={item.Image} alt="womenglass" />  </Link>
                </div>
                <div className="wogls-part">
                  <h1>{item.Title}</h1>
